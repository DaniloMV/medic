 ...
 */
class Lomedic_CatalogImport_Model_Import_Adapter_Xlsx extends Mage_ImportExport_Model_Import_Adapter_Abstract
{
    private $workbook;
    private $sheets;
    private $styles;
     ...
     * Init file import
     * 
     * @return \Lomedic_CatalogImport_Model_Import_Adapter_Xlsx
     */
    function _init() {
        $filename = $this->_source;
        $this->_unzip($this->_source);
        $this->_parse();
        $this->rows = $this->_optimizeRows();
        $this->rewind();
        return $this;
    }
    
    /**
     * Optimaze rows for import
     * 
     * @return array
     */
    private function _optimizeRows() {
        $rows = array();
        foreach ($this->rowsEx(key($this->sheets)) as $key=>$row) {
            foreach ($row as $k=>$_row) {
                if(!isset($rows[$key])) {
                    $rows[$key] = array();
                }
                $rows[$key][$k] = $_row['value']; 
            }
        }
        return $rows;
    }
    ...
    /**
     * Increase current number row
     */
    public function next() {
        $this->_currentRow = next($this->rows);
        $this->_currentKey = $this->_currentRow ? $this->_currentKey + 1 : null;
    }
    
    /**
     * Reset current number row
     */
    public function rewind() {
         // rewind resource, reset column names, read first row as current
        $this->_colNames = reset($this->rows);
        $this->_currentRow = next($this->rows);
    ...
    public function seek($position)
    {
        ...
        if ($position != $this->_currentKey) {
            if (0 == $position) {
                ...
                while ($this->_currentRow = $this->rows()) {
                    ...
                    if ( $this->workbook = $this->getEntryXML( $rel['Target'] )) {
                        if ( $workbookRelations = $this->getEntryXML( dirname($rel['Target']) . '/_rels/workbook.xml.rels' )) {
                            ...
                            foreach ($workbookRelations->Relationship as $workbookRelation) {
                                ...
                                } else if ($workbookRelation['Type'] == self::SCHEMA_REL_SHAREDSTRINGS && $this->entryExists( $path )) { // 0.6.6
                                ...
                                } else if ($workbookRelation['Type'] == self::SCHEMA_REL_STYLES) {
