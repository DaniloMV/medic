...
class Lomedic_Registration_Block_Rewrite_Adminhtml_Customer_Form_Element_File extends Mage_Adminhtml_Block_Customer_Form_Element_File
{
    ...
    protected function _getApproveRadioHtml()
    {
        ...
        //if ($this->getValue() && !$this->getRequired() && !is_array($this->getValue())) {
            ...
            $html .= '<span class="'.$this->_getApproveSpanClass().' approvefile">';
        ...
        $customerFilesCount = Mage::getModel('loregistration/files')->getCollection();
        $customerFilesCount->addFieldToFilter('customer_id',$customer->getId());

            if(isset($customerData["approve"]) && $customerData["approve"] == 0){
                $radio["checked"] = "checked";
            }elseif(!isset($customerData["approve"]) && $this->getValue() && !$this->getRequired() && !is_array($this->getValue())){
                $radio["checked"] = "checked";
            }
            ...
            $html .= '<span class="'.$this->_getApproveSpanClass().' notapprovefile">';
            $html .= $this->_drawElementHtml('input', $radio) . ' ';
